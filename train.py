...
class SequenceLightningModule(pl.LightningModule):
    def __init__(self, config):
        ...
        print(f'LighningModule init complete.')
    ...
    def setup(self, stage=None):
        ...
        print(f"Dataset loaded. Current cpu memory: {psutil.virtual_memory().percent}%")
        print(f"Dataset train: {len(self.dataset.dataset_train)} samples, val: {len(self.dataset.dataset_val)} samples, test: {len(self.dataset.dataset_test)} samples")
        ...
        print(f'Memory usage after model and task instantiation: {psutil.virtual_memory().percent}%')

        # Create encoders and decoders
        encoder = encoders.instantiate(
            encoder_cfg, dataset=self.dataset, model=self.model
        )
        print(f'Memory usage after encoder instantiation: {psutil.virtual_memory().percent}%')
        decoder = decoders.instantiate(
            decoder_cfg, model=self.model, dataset=self.dataset
        )
        print(f'Memory usage after decoder instantiation: {psutil.virtual_memory().percent}%')
