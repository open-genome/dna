...
class LinearLRSchedulerWarmup(Scheduler, torch.optim.lr_scheduler._LRScheduler):
    """ Wrap timm.scheduler.CosineLRScheduler so we can call scheduler.step() without passing in epoch.
    It supports resuming as well.
    """

    def __init__(self, *args, **kwargs):
        self.t_initial = kwargs.pop("t_initial")
        self.warmup_t = kwargs.pop("warmup_t")
        self.warmup_lr_init = kwargs.pop("warmup_lr_init")
        self.lr_min = kwargs.pop("lr_min")
        keys = [i for i in kwargs.keys()]
        for name in keys:
            if name != "t_in_epochs":
                kwargs.pop(name)
        super().__init__(*args, param_group_field="lr", **kwargs)
        self.step(epoch=0)

    def _get_lr(self, t):
        if t < self.warmup_t:
            # Linear warmup
            lrs = [self.warmup_lr_init + t * (lr - self.warmup_lr_init) / self.warmup_t for lr in self.base_values]
        else:
            if self.cycle_limit is not None and t >= self.cycle_limit * self.t_initial:
                # If we've reached the limit of cycles, use the minimum learning rate.
                lrs = [self.lr_min for _ in self.base_values]
            else:
                # Linear decay
                cycle = math.floor(1 + (t - self.warmup_t) / self.t_initial)
                t_curr = t - self.warmup_t - (cycle - 1) * self.t_initial
                gamma = self.cycle_decay ** cycle
                lr_max_values = [v * gamma for v in self.base_values]
                lrs = [
                    self.lr_min + (lr_max - self.lr_min) * (1 - t_curr / self.t_initial)
    ...
    def _get_closed_form_lr(self):
        # This method is not used since we implement get_lr directly.
        pass