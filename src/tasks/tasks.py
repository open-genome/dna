    ...
    def _init_torchmetrics(self):
        ...
        for name in self.torchmetric_names:
            if name in tm_mine:
                ...
                if name=="F1":
                    name="F1Score"
                tracked_torchmetrics[name] = getattr(tm, name)(average='macro', num_classes=self.dataset.d_output, compute_on_step=False, task=self.dataset.task)
            elif '@' in name:
                k = int(name.split('@')[1])
                mname = name.split('@')[0]
...
class LMTask(BaseTask):
    def forward(self, batch, encoder, model, decoder, _state):
        ...
        x = x.logits
        if isinstance(x, tuple): # x come out from bert lm head
            x = list(x)
            x[0] = rearrange(x[0], '... C -> (...) C')
        else:
            x = rearrange(x, '... C -> (...) C')
        y = rearrange(y, '... -> (...)')
