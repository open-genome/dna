...
from src.dataloaders.datasets.hg38_dataset import HG38Dataset, BertHG38Dataset
...
from src.dataloaders.datasets.hg38_fixed_dataset import HG38FixedDataset, BertHG38FixedDataset
from src.dataloaders.datasets.deepstarr import DeepSTARRDataset
from src.dataloaders.datasets.deepsea import DeepSEADataset
...
class GenomicBenchmark(HG38):
    ...
    def __init__(self, dataset_name, dest_path=None, tokenizer_name='char', d_output=None, rc_aug=False,
        ...
        self.pin_memory = pin_memory
        self.drop_last = drop_last
        if "use_tokenizer" in kwargs:
            self.use_tokenizer = kwargs["use_tokenizer"]
        else:
            self.use_tokenizer = None
    ...
    def setup(self, stage=None):
        ...
        self.dataset_train, self.dataset_val = [
            GenomicBenchmarkDataset(split=split,
                                ...
                                use_tokenizer=self.use_tokenizer if self.use_tokenizer is not None else True
            )
...
class DeepSTARR(HG38):
    _name_ = "deepstarr"
    l_output = 0  # need to set this for decoder to work correctly

    def __init__(self, dataset_name, dest_path=None, tokenizer_name='char', d_output=None, rc_aug=False,
                max_length=1024, use_padding=True, max_length_val=None, max_length_test=None, label_to_token=None,
                add_eos=True, characters=None, padding_side='left', val_ratio=0.0005, val_split_seed=2357,
                detokenize=False, val_only=False, batch_size=32, batch_size_eval=None, num_workers=0,
                shuffle=True, pin_memory=False, drop_last=False, fault_tolerant=False, ddp=False,
                fast_forward_epochs=None, fast_forward_batches=None,
    ...
    def init_datasets(self):
        ...
        # delete old datasets to free memory
        if hasattr(self, 'dataset_train'):
            del self.dataset_train.data

        # delete old datasets to free memory
        if hasattr(self, 'dataset_test'):
            del self.dataset_test.data
    
        # Create all splits: torch datasets
        self.dataset_train, self.dataset_val, self.dataset_test = [
            DeepSTARRDataset(split=split,
                            max_length=max_len,
                            tokenizer=self.tokenizer,  # pass the tokenize wrapper
                            tokenizer_name=self.tokenizer_name,
                            add_eos=self.add_eos,
                            rc_aug=self.rc_aug,
                            return_augs=False,
                            d_output=self.d_output, # default binary classification
                            dest_path=self.dest_path,
                            use_padding=self.use_padding,
                            use_tokenizer=self.use_tokenizer if self.use_tokenizer is not None else True
                        )
...
class DeepSEA(HG38):
    ...
    def setup(self, stage=None):
        ...
        self.dataset_train, self.dataset_val, self.dataset_test = [
            ...
            for split, max_len in zip(splits, [self.max_length, self.max_length_val, self.max_length_test])
        ]
...
class BertHG38(SequenceDataset):
    """
    Base class, other dataloaders can inherit from this class.
    ...
    """
    ###### very important to set this! ######
    _name_ = "bert_hg38"  # this name is how the dataset config finds the right dataloader
    #########################################
    ...
    def __init__(self, bed_file, fasta_file, tokenizer_name=None, dataset_config_name=None, max_length=1024, d_output=2, rc_aug=False,
                 ...
                 add_eos=True, detokenize=False, val_only=False, batch_size=32, batch_size_eval=None, num_workers=1,
                 shuffle=False, pin_memory=False, drop_last=False, fault_tolerant=False, ddp=False,
                 fast_forward_epochs=None, fast_forward_batches=None, replace_N_token=False, pad_interval=False, use_tokenizer=True, pad_max_length=None,
                 objective="stdmlm",
                 *args, **kwargs):
        self.dataset_config_name = dataset_config_name
        self.tokenizer_name = tokenizer_name
    ...
    def init_datasets(self):
        ...
        if self.use_fixed_len_val:
            ...
            self.dataset_val = BertHG38FixedDataset(
                chr_ranges=chr_ranges,
                fasta_file=self.fasta_file,
                max_length=self.max_length,
                pad_max_length=self.max_length,
...
class BertHG38Fixed(HG38):
    _name_ = "bert_hg38_fixed"
    ...
    def setup(self, stage=None):
        ...
        # we only need one
        self.dataset_train = BertHG38FixedDataset(
            fasta_file=self.fasta_file,
            chr_ranges=self.chr_ranges,  # a dict of chr: (start, end) to use for test set
            max_length=self.max_length,
            pad_max_length=self.pad_max_length,
...
class DNABert2Pretrain(SequenceDataset):
    """
    Base class, other dataloaders can inherit from this class.
    ...
    """
    ###### very important to set this! ######
    _name_ = "dnabert2_pretrain"  # this name is how the dataset config finds the right dataloader
    #########################################

    def __init__(self, text_file, tokenizer_name=None, dataset_config_name=None, max_length=1024, d_output=2, rc_aug=False,
                 max_length_val=None, max_length_test=None, val_ratio=0.0005, val_split_seed=2357, use_fixed_len_val=False,
                 add_eos=True, detokenize=False, val_only=False, batch_size=32, batch_size_eval=None, num_workers=1,
                 shuffle=False, pin_memory=False, drop_last=False, fault_tolerant=False, ddp=False,
                 fast_forward_epochs=None, fast_forward_batches=None, replace_N_token=False, pad_interval=False, use_tokenizer=True,
                 objective="stdmlm",
                 *args, **kwargs):
        self.dataset_config_name = dataset_config_name
        self.tokenizer_name = tokenizer_name
    ...
    def init_datasets(self):
        ...
        print("Validation dataset size:", len(self.dataset_val))
        print("Test dataset size:", len(self.dataset_test))
...
class DNABertSPretrain(SequenceDataset):
    """
    Base class, other dataloaders can inherit from this class.
    ...
    """
    ###### very important to set this! ######
    _name_ = "dnaberts_pretrain"  # this name is how the dataset config finds the right dataloader
    #########################################

    def __init__(self, text_file, seq_name='seq_a', tokenizer_name=None, dataset_config_name=None, max_length=1024, d_output=2, rc_aug=False,
    ...
    def init_datasets(self):
        ...
        self.dataset_train, self.dataset_val, self.dataset_test = [
            DNABERTSDataset(split=split,
                        ...
                        use_tokenizer=self.use_tokenizer,
                        objective=self.objective)
            for split, max_len in zip(['train', 'val', "val"], [self.max_length, self.max_length_val, self.max_length_test])
        ] 
