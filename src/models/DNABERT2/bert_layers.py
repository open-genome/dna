...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        alibi_bias = self.alibi[:, :, :seqlen, :seqlen].type(extended_attention_mask.dtype)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.hyena_framework:
            ...
            attention_mask = input_ids!=AutoTokenizer.from_pretrained(root+"/DNABERT-2-117M").pad_token_id
            labels = batch[2]
        if (input_ids is not None) == (inputs_embeds is not None):
            raise ValueError('Must specify either input_ids or input_embeds!')
        ...
        if labels is not None:
            ...
            B, seqlen = input_ids.shape[:2]
            prediction_scores = rearrange(index_put_first_axis(
                prediction_scores, masked_token_idx, B * seqlen),
                                          '(b s) d -> b s d',
                                          b=B)
        ...
        if self.hyena_framework:
            return MaskedLMOutput(
                loss=loss,
                logits=(prediction_scores, batch[1]),
                hidden_states=None,
                attentions=None,
            ), None
        else:
